...
async def on_message(message):
  ...
  if msg.lower().startswith('?micheal') or msg.lower().startswith('?mike'):
    quote = get_quote()
    ...
    await message.channel.purge(limit=1)
    await message.channel.sen("https://tenor.com/view/embarrassed-michaelscott-theoffice-gif-5120730")

  elif msg.lower().startswith('noo')or msg.lower().startswith('toby'):
    await message.channel.purge(limit=1)
    ...
    embedVar.add_field(name="Gifs keywords", value="cringe, kms, kill you, noo, vance, huh, white, broke", inline=False)
